    ...
    if (!$adminMenu.length && settings.admin_menu.hash) {
      Drupal.admin.getCache(settings.admin_menu.hash, function (response) {
          if (typeof response == 'string' && response.length > 0) {
            ...
            window.localStorage.setItem('admin_menu_cache', response);
            window.localStorage.setItem('admin_menu_cache_hash', settings.admin_menu.hash);
          }
          var $adminMenu = $('#admin-menu:not(.admin-menu-processed)', context);
          // Apply our behaviors.
...
Drupal.admin.getCache = function (hash, onSuccess) {
  ...
  if (hash === window.localStorage.getItem('admin_menu_cache_hash') && hash) {
    var localStorageCache;
    if (localStorageCache = window.localStorage.getItem('admin_menu_cache')) {
      onSuccess(localStorageCache);
      return;
    }
  }
  $.ajax({
